...
| `-jN` | off | check in two passes, the second on `N` threads (bare `-j`: one per core). Pass one admits each declaration on its statement; pass two checks the values, which is where four fifths of the time goes and which nothing else depends on. The file is read on a thread of its own alongside pass one. Same verdict either way — SPEC §11.6 says why. `std` in 27s rather than 139s. |
