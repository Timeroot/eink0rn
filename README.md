...
| --- | --- | --- | --- | --- | --- |
| `init.ndjson` (325 MB) | 53,093 | ACCEPT | 1m 07s | 1.8 GB | 4.7s |
| `std.ndjson` (552 MB) | 90,778 | ACCEPT | 2m 13s | 2.7 GB | 9.4s |
| `cslib.ndjson` (2.1 GB) | 370,939 | ACCEPT | 8m 04s | 6.5 GB | 34.1s |
| `mathlib.ndjson` (5.6 GB) | 654,504 | ACCEPT | 44m 00s | 24.2 GB | 5m 16s |

The last column is the same run at `-j32 +RTS -A128m`, on all 64 cores rather
than one. For the three smaller corpora that is the time it takes to *read* the
file and nothing more (SPEC §11.6).
